...
function initBookingModal() {
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', (e) => {
      ...
      const bookingId = 'GLW-' + Math.floor(100000 + Math.random() * 900000);
      ...
      document.getElementById('summaryDetails').innerHTML = `
        <div class="receipt-box">
          ...
          <div class="receipt-footer">
            A confirmation SMS & WhatsApp reminder has been sent to <strong>+91 ${phone}</strong>.
          </div>
        </div>
      `;
      ...
      if (typeof confetti === 'function') {
        confetti({
          ...
          spread: 80,
          origin: { y: 0.55 },
          colors: ['#C59B63', '#FAF5EB', '#1A1715', '#E0C189']
        });
      }
    });
  }
...
function saveBookingToStorage(booking) {
  try {
    let bookings = JSON.parse(localStorage.getItem('glowence_bookings') || '[]');
    bookings.unshift(booking);
    localStorage.setItem('glowence_bookings', JSON.stringify(bookings));
    checkSavedBookings();
  } catch(e) {
    console.error(e);
  }
